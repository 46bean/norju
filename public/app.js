...
function applyIdentity(name, display) {
  ...
  localStorage.setItem('norju_nickname', name);
  localStorage.setItem('norju_myname', state.myName);
...
function setNickChoice(v) {
  nickChoice = v;
  document.querySelectorAll('.nick-opt').forEach((b) =>
    b.classList.toggle('on', b.dataset.nick === v));
  const other = v === '__other';
  $('#nick-other-wrap').hidden = !other;
  if (other) setTimeout(() => $('#nick-input').focus(), 0);
}

$('#nick-choices').addEventListener('click', (e) => {
  const b = e.target.closest('.nick-opt');
  if (!b) return;
  setNickChoice(b.dataset.nick);
...
function askNickname() {
  ...
  $('#nick-error').textContent = '';
  if (PRESET_NICKS.includes(state.nickname)) {
    $('#nick-input').value = '';
    setNickChoice(state.nickname);
  } else if (state.nickname) {
    $('#nick-input').value = state.nickname;
    setNickChoice('__other');
  } else {
    $('#nick-input').value = '';
    setNickChoice(null);
  }
...
$('#form-nick').addEventListener('submit', (e) => {
  let v = '';
  if (nickChoice === '__other') v = $('#nick-input').value.trim().slice(0, 20);
  else if (PRESET_NICKS.includes(nickChoice)) v = nickChoice;

  if (!v) {
    e.preventDefault();
    $('#nick-error').textContent =
      nickChoice === '__other' ? '닉네임을 입력해 주세요.' : '이름을 선택해 주세요.';
    return;
  }
...
/* ---------- 시작 ---------- */
(function init() {
  const saved = localStorage.getItem('norju_nickname');
  const savedName = localStorage.getItem('norju_myname') || '';
